...
def analyze_packets_and_assign_roles_optimized(file_path):
    ...
    for ip, obs in ip_data.items():
        current_roles = set()
        current_reasoning = obs['reasoning_parts'].copy() # Use pre-set reasoning from internal rules
        ...
        final_results.append({
            'ip': ip,
            'roles': sorted(list(current_roles.union(obs['roles']))), # Merge roles set during hierarchical inference
            'reasoning': " ".join(sorted(list(current_reasoning.union(obs['reasoning_parts'])))).strip() # Merge reasoning
        })
