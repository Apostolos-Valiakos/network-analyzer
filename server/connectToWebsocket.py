...
def send_packets_to_flask(chunk, chunk_index, total_chunks, is_final_chunk, current_session_id):
    ...
    while attempt < MAX_RETRIES:
        attempt += 1
        try:
            print(f"Sending chunk {chunk_index + 1}/{total_chunks} (Attempt {attempt})...")
            ...
            if response.ok and data.get('success'):
                ...
                error_message = data.get('error') or f"Unknown API error: {response.status_code}"
            ...
            if attempt == MAX_RETRIES:
                raise Exception(f"Failed to process chunk {chunk_index + 1} after {MAX_RETRIES} attempts. Last error: {e}")
...
def call_automated_analysis_endpoint_server_side(ws, pcap_filename):
    ...
    try:
        response = requests.get(analysis_url, params=params, headers=headers, timeout=180) 
        analysis_data = None
...
def execute_pipeline(ws_url, ws, capture_duration): # USED capture_duration
    ...
    try:
        ...
        if not filename:
             raise Exception("PCAP file was not created on the server (no filename returned).")
...
def startWebSocketClient(ws_url, capture_duration): # ADDED capture_duration
    ...
    pipeline_thread = threading.Thread(target=execute_pipeline, args=(ws_url, ws, capture_duration,), daemon=True) 
